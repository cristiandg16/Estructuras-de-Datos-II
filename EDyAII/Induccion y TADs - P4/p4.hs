...
--1) Especificacion algebraica para listas finitas:
...
--	c) 	
		...
		inL Nil y = False
		inL (cons x xs) y = x == y || inL xs y

--	d) 
		borraOcurs :: List A -> A -> List A

		borraOcurs Nil y = Nil
		borraOcurs (cons x xs) y = if x == y then borraOcurs (tail xs) y
									else cons x (borraOcurs xs y)
...
--2)
	...
	tad Pila (a:Set) where
		...
		Empty  = < > 
		push < x1, x2, x3, ... > x = < x, x1, x2, x3, ... >

		isEmpty < x1, x2, x3, ... > = if n == 0 then True
										else False
...
--3)
		...
		borrar x (insertar y c) = if x == y then borrar x c
		...
		union _ vacio = c
		union c1 (insertar x c2) = union (insertar x c1) c2

		interseccion c1 vacio = vacio
		interseccion c1 (insertar x c2) = if (pertenece x c1) == True then insertar x (interseccion c1 c2)
										  else interseccion c1 c2 
		
		pertence :: a -> Conjunto a -> Bool

		pertenece x (insertar y c) | x == y = True
  		  						   | otherwise = pertenece x c  

		resta c vacio = c
		resta c1 (insertar x c2) | ((pertenece x c1) && (pertenece x c2)) == True = borrar x c1
								 | otherwise = resta c1 c2
...
--5)
		...
		tad BalT (A:Ordered Set) where
			...
			expose	: BalT a -> Maybe (BalT a, a, BalT a)
		 	...
		 	size empty = 0
		 	size (join l x r) = if x == Nothing then size l + size r
		 						else size l + 1 + size r
...
{-
	Vamos a realizar la prueba mediante inducción.
	Caso Base, []:
	...
	Paso inductivo, ps:
		...
		((uncurry zip) . unzip) ps 
	= < Definicion de . >
		(uncurry zip) (unzip ((x,y),ps))
	= < Definicion de unzip.2 >
